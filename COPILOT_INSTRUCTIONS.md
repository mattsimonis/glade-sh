...
40. **Reconnect stuck on "Reconnecting…" after app backgrounding** — iOS freezes all JavaScript timers when a WKWebView app is backgrounded. A 30s safety timeout scheduled at t=0, frozen at t=5s on background, thaws and fires at ~t=60s — by which time a new `forceReconnect()` may have already started. The thawed timer fires, clears `reconnectInProgress` mid-reconnect, and state is left in `reconnecting` with no retry loop. Fix: guard the safety timeout with a reconnect ID (`_safetyRId === _forceReconnectId`) so stale frozen timers are no-ops. A second escape hatch (`_reconnectingStuckTimer`) fires after 15s whenever `reconnecting` state is entered — if still stuck, it calls `forceReconnect()` again. Without this, the `reconnecting` state had no self-recovery path.
