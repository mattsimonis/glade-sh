...
- **Service worker** — `res.clone()` must be called synchronously before any async gap; was throwing `TypeError: Failed to execute 'clone' on 'Response': Response body is already used`; cache bumped to `glade-shell-v3`
- **`toggleDesktopPanel` scope** — was defined inside an IIFE but called from outer keydown handler; fixed via `window.toggleDesktopPanel = toggleDesktopPanel`
...
- **iOS 18+ haptic feedback** — `haptic()` detects iOS via userAgent and fires a light haptic using a hidden `<input type="checkbox" switch>` + `<label>` trick; Android still uses `navigator.vibrate`; older browsers silent no-op
...
- **Multi-project terminals** — per-project tmux sessions with isolated ttyd instances on ports 7690–7729
...
- **Tailscale support** — remote access over mesh VPN
