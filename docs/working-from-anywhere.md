...
Your server runs the hub. Every device reaches it through Tailscale.
The terminal is a ttyd session inside Docker — your tools run there.

The gap: your project files live on your laptop, not the server.
...
|--------|--------------|---------|
| **SSH from hub into laptop** | server terminal SSHes to your laptop. Files, git, tools all stay on laptop. | Working on an active local project |
...
| **Clone project to server** | Manually clone/sync to the server. Work from there. | Repos not on GitHub, or custom sync workflows |
...
The terminal on your server can SSH into your laptop over Tailscale. Once connected, you're working on your laptop's filesystem — your files, your git history, your tools, your `gh auth`.
...
### One-time setup: Add your server's key to your laptop

This avoids password prompts. Run this from **inside the Glade terminal** (on the server):

```sh
# Generate a key if one doesn't exist
ssh-keygen -t ed25519 -C "glade@server" -f ~/.ssh/id_ed25519 -N ""
...
4. When done: `exit` returns you to the server shell.
...
Better: add a shell alias to the server's `~/.zshrc` inside the Docker container:
```sh
...
4. Glade clones it to `~/.glade/projects/{slug}` on the server and opens a terminal in it

From there, `git pull` and `git push` work normally inside the terminal. The repo lives on the server, survives container restarts, and is accessible from any device.

---

## Option 3: Clone / Sync Project to Server (Manual)

If you want the project to live on the server itself (no SSH hop):

### Initial clone

```sh
# Inside Glade terminal (on the server)
mkdir -p ~/projects
...
If you're actively editing on your laptop AND want the server terminal to have the latest:
...
# On server: pull when needed
git pull
```

**Option B — Live rsync** (real-time, no git needed)
```sh
# On laptop, run this — it watches for file changes and rsyncs to server
...
Mutagen syncs in real-time, both directions. Edit on laptop, run on server, changes appear everywhere. Requires files `node_modules` to be excluded via a mutagen config.
...
No setup needed on the server. Just SSH to your laptop and `cd` to the project. Create a Glade project entry as a reminder/shortcut.

Add an alias inside the Docker container:
```sh
# In server's ~/.zshrc (edit via docker exec or rebuild)
...
2. **Leave the Glade terminal running** — tmux sessions inside Docker persist. You don't need to do anything special. The session stays alive.
3. **If you were working in the Glade terminal** (not SSH'd into laptop): already done. Your tmux session is there.
...
3. If your session was in the Glade terminal: it's exactly where you left it (tmux)
...
Tools installed in your Glade container are available in the Glade terminal. When you SSH into your laptop, you use the laptop's own tools instead.
...
- On the host (via Glade Settings): connect via Settings → GitHub
...
| SSH key from server | Action needed | `ssh-copy-id` (one-time, 2 minutes) |
...
- [ ] From the Glade terminal: `ssh-keygen` then `ssh-copy-id matt@matts-macbook`  
- [ ] Test: from Glade terminal, `ssh matt@matts-macbook` — should connect without password
...
**SSH times out from Glade terminal**
...
- To use server tools, work from the Glade terminal and sync files (Option 3).

**Files out of sync between laptop and server**
- Option 1 (SSH) doesn't have this problem — you're editing directly on the laptop
- Option 2: run `git pull` on server or re-run your rsync
